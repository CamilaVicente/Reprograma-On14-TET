...
function buscarPreco(produto) {
  ...
  resolver = async (produto, parcelasDesejadas) => {
    try {
      ...
      console.log(`Seu notebook custa R$${valorTotal.replace('.', ',')} e você pagará em ${parcelasDesejadas}x de R$${valorFinalDaParcela.replace('.', ',')}`)
    }
    catch(err) {
  ...
  Você quer saber quanto vai pagar em reais por um produto comprado nos EUA e para isso precisa consultar numa "API" de contação para descobrir o valor do Dólar no momento da compra (você deve usar o valor do dólar comercial) e calcular o valor em Real, em seguida precisa consultar outra "API" que retorna o valor de dois juros que serão cobrados sob o preço em Real e retornar o valor final
